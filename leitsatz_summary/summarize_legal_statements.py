...
from leitsatz_summary.summarize_CaseSummarizer import param_pos_para_function, param_nesting_function, \
    param_branching_function, param_para_number_function, param_dates, param_entities, param_legal_entities, \
    param_section_begin, param_ignore_tense, select_params_to_df, get_tfidf_dicts, do_one_judgement
...
def visualize_eval_results(model_id, test_df, eval_result_path, picture_path):
    ...
    if model_id not in [mod_random, mod_old]:
...
def save_texts_for_eval(test_df):
    ...
    for model_id in [mod_random, mod_old]:
...
def calculate_best_parameters(dataframe):
    """
    Determines the best parameters for the old algorithm with a grid search. Saves the results to a file.

    :param dataframe: Data to work on
    """
    possible_param_values = {param_pos_para_function: np.arange(-6, 7, 1),
                             param_nesting_function: np.arange(-6, 7, 1),
                             param_branching_function: np.arange(-6, 7, 1),
                             param_para_number_function: np.arange(-6, 7, 1),
                             param_dates: np.arange(-6, 7, 1),
                             param_entities: np.arange(-6, 7, 1),
                             param_legal_entities: [False, True],
                             param_section_begin: np.arange(-6, 7, 1),
                             param_ignore_tense: [False, True]}
    df = dataframe.set_index(aktenzeichen_str)
    select_params_to_df(data=df, params=possible_param_values)


def apply_old_approach(dataframe):
    """
    Applied the old approach and saves the created leitsaetze.

    :param dataframe: Data to create leitsaetze for
    """
    param_weights_now = {param_nesting_function: -6,
                         param_pos_para_function: -2,
                         param_para_number_function: -2,
                         param_branching_function: -1,
                         param_dates: 0, param_entities: 0,
                         param_section_begin: 0,
                         param_legal_entities: False, param_ignore_tense: False}

    tfidf_dict, df_dict = get_tfidf_dicts()
    package = []
    for ind, row_data in dataframe.iterrows():
        package.append([row_data[aktenzeichen_str], row_data, tfidf_dict, param_weights_now])
    ranking = utils.parallel_imap(do_one_judgement, package)
    ranking = [[' '.join([leitsatz['original_sentence'] for leitsatz in leitsatz_sents]),
               ' '.join([res['original_sentence'] for res in res_sents])] for [res_sents, leitsatz_sents, _] in ranking]
    ls = [leitsatz for [leitsatz, _] in ranking]
    summaries = [res for [_, res] in ranking]
    res_df = pd.DataFrame.from_dict({leitsatz_str: ls, summary_str: summaries})

    utils.create_dir(current_path=current_path, directory_name=eval_result_path_simple, delete=False)
    utils.df_to_json(current_path=current_path, path=eval_result_path_simple + mod_old, dataframe=res_df)


def create_simple_leitsaetze(train_df, test_df):
    """
    Wrapper methof for the old approach. Creates the leitsaetze, evaluates them and plots figures.

    :param train_df: Train data to tune parameters.
    :param test_df: Test data to create leitsaetze for
    """
    #calculate_best_parameters(train_df)
    #apply_old_approach(test_df)
    #evaluate(model_id=mod_old, eval_result_path=eval_result_path_simple)
    visualize_eval_results(model_id=mod_old, eval_result_path=eval_result_path_simple, test_df=test_df,
                           picture_path=picture_path_simple)
    print('Done creating simple leitsaetze.')
...
def visualize_manual_eval():
    ...
    for model in ['eval_results/finetune_for_leitsatz/' + elem for elem in list(fine_tune_models.keys())] + \
                 ...
                 [eval_result_path_simple+mod_old, eval_result_path_simple+mod_random]:
    ...
    axes = print_df[[mod_old, mod_random] + list(fine_tune_models.keys())].plot(rot=90)
    fig = axes.get_figure()
    fig.tight_layout()
    fig.savefig(picture_path_manual_results + save_extension + 'metrics_finetune.png')
    plt.clf()

    axes = print_df[[mod_old, mod_random] + list(instruct_models.keys())].plot(rot=90)
    fig = axes.get_figure()
    fig.tight_layout()
    ...
    for model in list(fine_tune_models.keys()) + list(instruct_models.keys()) + ['', mod_old, mod_random]:
        dataframe = df
        if model != '':
...
def correlation_scores_classes(test_df):
    ...
    for model_id in [[model] for model in res_df[model_str].unique()]+[res_df[model_str].unique()]:
        print(model_id)
        corr_df = res[res[model_str].isin(model_id)].corr('spearman')
...
if __name__ == "__main__":
    ...
    create_simple_leitsaetze(test_df=test, train_df=train)
